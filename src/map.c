 ...
 * @return    Renvoie la position du point de départ sous forme de coordonnées,
 * négatives si non trouvés
 */
extra_coords map_getEntry(map *in) {
  for (int i = 0; i < in->h; i++) {
    for (int j = 0; j < in->w; j++) {
      if (in->data[i][j] == 'D')
        return extra_new_coords(j, i);
    }
  }
 ...
 * @param  in Carte à analyser
 * @return    Renvoie la position du point d'arrivé sous forme de coordonnées,
 * négatives si non trouvés
 */
...
/**
 * Vérifie si le caractère c est le meme que celui de la carte à la position
 * (x,y)
 * @param  in Carte à analyser
 ...
 * @param in Carte à libérer en mémoire
 */
